...
```python
"""
Frontend weight/volume categories (UI select options).

Важно: фактический CargoTech API использует query параметр `filter[wv]`
в формате "weight-volume" (например: `15-65`), а не `weight_min/max` и
`volume_min/max`.
"""
...
def normalize_weight_volume_filter(value: str) -> dict:
    """
    Convert weight/volume filter to CargoTech API format.

    Args:
        value: "1_3", "3_5", "5_10", "10_15", "15_20", "20", etc. (frontend format)

    Returns:
        {"filter[wv]": "15-65"} (API format) or {} if value is "any"/empty.
    ...
    api_value = API_FORMAT_MAP.get(value)
    if not api_value:
        raise ValidationError(f"Invalid weight_volume value: {value}")

    return {"filter[wv]": api_value}
```
...
## 7) Где это связано с актуальными контрактами v3.1
